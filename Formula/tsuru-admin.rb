...
class TsuruAdmin < Formula
  ...
  def install
    system "bash", "-c", "[ $( go version|awk '{print $3}' | cut -d' ' -f3|sed 's/go//; s/\.//g; s/^\(..\).*/\1/') -lt 11 ] && echo ERROR: tsuru-admin requires Go 1.1 or later, your version is: $(go version) && exit 1"
