...
class Tsuru < Formula
  ...
  def install
    system "bash", "-c", "test $( go version|awk '{print \$3}' | sed 's/go//; s/\\.//g; s/^\\(..\\).*/\\1/') -lt 11 && echo ERROR: tsuru requires Go 1.1 or later, your version is: $(go version) && exit 1 || echo proceeding ..."
